...
export type DitherAlgorithm = "bayer" | "atkinson" | "floyd-steinberg" | "threshold";
...
export interface DitherImageProps {
  ...
  width?: number;
  /** Called with the dithered canvas after a successful pass. */
...
const luminanceOf = ([r, g, b]: Rgb) => (r * LUMA_R + g * LUMA_G + b * LUMA_B) / MAX_CHANNEL;

const channelAt = (data: Uint8ClampedArray | Float32Array, index: number) => data[index] ?? 0;

const readCssColor = (scratch: CanvasRenderingContext2D, value: string): Rgb => {
  scratch.clearRect(0, 0, 1, 1);
  scratch.fillStyle = value;
  scratch.fillRect(0, 0, 1, 1);
  const { data } = scratch.getImageData(0, 0, 1, 1);
  return [channelAt(data, 0), channelAt(data, 1), channelAt(data, 2)];
};

const readThemeStops = (scratch: CanvasRenderingContext2D, element: HTMLElement): Rgb[] => {
  const styles = getComputedStyle(element);
  const foreground = styles.getPropertyValue("--color-foreground").trim() || FALLBACK_DARK;
  const background = styles.getPropertyValue("--color-background").trim() || FALLBACK_LIGHT;
  const first = readCssColor(scratch, foreground);
  const second = readCssColor(scratch, background);
  return luminanceOf(first) <= luminanceOf(second) ? [first, second] : [second, first];
};

const buildRamp = (stops: Rgb[], levels: number): Rgb[] => {
  const first = stops[0] ?? [0, 0, 0];
  if (stops.length <= 1) return Array.from({ length: levels }, () => first);
  ...
  return Array.from({ length: levels }, (_unused, index) => {
    ...
    const from = stops[lower] ?? first;
    const to = stops[upper] ?? from;
    return [
...
const readLuminance = (data: Uint8ClampedArray) => {
  ...
  for (let index = 0; index < values.length; index++) {
    ...
    values[index] =
      (channelAt(data, offset) * LUMA_R +
        channelAt(data, offset + 1) * LUMA_G +
        channelAt(data, offset + 2) * LUMA_B) /
      MAX_CHANNEL;
  }
  return values;
};
...
const diffuse = (
  ...
  for (let y = 0; y < gridHeight; y++) {
    for (let x = 0; x < gridWidth; x++) {
      ...
      const original = channelAt(values, index);
      ...
      for (const tap of kernel) {
        ...
        values[neighbor] = channelAt(values, neighbor) + error * tap.weight;
      }
    }
  }
  return quantised;
...
const quantiseOrdered = (values: Float32Array, gridWidth: number, levels: number, ordered: boolean) => {
  const steps = levels - 1;
  const quantised = new Uint8Array(values.length);
  for (let index = 0; index < values.length; index++) {
    let value = channelAt(values, index);
    if (ordered) {
      const x = index % gridWidth;
      const y = Math.floor(index / gridWidth);
      const cell = BAYER_MATRIX[(y % BAYER_SIZE) * BAYER_SIZE + (x % BAYER_SIZE)] ?? 0;
...
const applyRamp = (data: Uint8ClampedArray, quantised: Uint8Array, ramp: Rgb[]) => {
  for (let index = 0; index < quantised.length; index++) {
    const offset = index * CHANNELS;
    const tone = ramp[quantised[index] ?? 0] ?? ramp[0] ?? [0, 0, 0];
    data[offset] = tone[0];
...
const DitherImage = ({
  ...
  const resolvedPalette = useMemo(() => (paletteKey ? paletteKey.split("|") : undefined), [paletteKey]);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (algorithm === "floyd-steinberg") {
        quantised = diffuse(values, gridWidth, gridHeight, safeLevels, FLOYD_STEINBERG_KERNEL);
      } else if (algorithm === "atkinson") {
        quantised = diffuse(values, gridWidth, gridHeight, safeLevels, ATKINSON_KERNEL);
      } else {
        quantised = quantiseOrdered(values, gridWidth, safeLevels, algorithm === "bayer");
      }
  ...
  }, [algorithm, height, resolvedPalette, safeLevels, safePixelSize, src, width]);
  ...
  return (
    <div
      className={cn("relative isolate overflow-hidden rounded-xl bg-background", className)}
      ref={containerRef}
      style={{ height, width }}
    >
      <img alt={alt} className="absolute inset-0 h-full w-full object-cover" height={height} src={src} width={width} />
      <motion.canvas
        animate={{
        ...
        transition={shouldReduceMotion ? { duration: 0 } : { duration: REVEAL_DURATION_S, ease: REVEAL_EASE }}
      />
    </div>
  );
};
